...
export default function QuestionnairePage() {
  ...
  const shuffledQuestionnaire = useMemo(() => {
    ...
    shuffled.questions = questionnaire.questions.map((q: any) => {
  ...
  const handleSubmit = useCallback(
    async (answersToSubmit?: number[]) => {
      ...
      try {
        ...
        const responses = questionnaire.questions.map((q: any, idx: number) => ({
          questionId: q.id,
  ...
  const handlePastedAnswersSubmit = useCallback(() => {
    ...
    try {
      ...
      alert(e.message || "Failed to submit pasted answers. Please check them.");
    }
