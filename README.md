...
2. **Run the migration(s):** In the Supabase **SQL Editor**, run [`001_employee_validation_sessions.sql`](supabase/migrations/001_employee_validation_sessions.sql) (creates the table), then [`003_employee_validation_sessions_followup_fields.sql`](supabase/migrations/003_employee_validation_sessions_followup_fields.sql) (adds `actual_career_field`, `rating_top1`–`rating_top3`). If you previously created an older, wider version of this table, run [`002_employee_validation_sessions_drop_legacy_columns.sql`](supabase/migrations/002_employee_validation_sessions_drop_legacy_columns.sql) once before `003`. Inserts use the **service role** from your API route only (RLS enabled).
